...
int main() {
cout<< "Version 1: "<<pow1(2,5)<<endl;
cout << "Number of recursive call: "<< k1<<endl;
cout<< "Version 2: "<<pow2(2,5)<<endl;
cout << "Number of recursive call: "<< k2<<endl;
}
...
int pow2(int a,int n){
  ...
  if (n==0){return 1;}
    ...
    if(n%2==0){return pow2(a,n/2)*pow2(a,n/2);}
    else{return a*pow2(a,n/2)*pow2(a,n/2);}
  }
}
//Running the code multiple time we observe that the first version needs n+1 recursive calls to compute the power. The second versuon needs approxiamtly (depending on the case) 3n recursive calls to compute the power. We understand that by looking at the recursive definition.
*/
